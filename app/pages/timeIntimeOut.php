    ...
    if(isset($_POST['submit'])) {
        ...
        if(mysqli_num_rows($result) > 0) {
            ...
            if(mysqli_num_rows($attendance_result) > 0){
                ...
                // If no existing record, insert a new attendance record with default time out
                $default_time_out = ($time_suffix == 'AM') ? '12:00 PM' : '05:00 PM'; // Default time out for AM and PM
                $sql = "INSERT INTO attendance(id_number, time_in, date, period, purpose_id, schoolyear, status) VALUES(?, ?, ?, ?, ?, ?, '0')";
                $stmt = mysqli_prepare($connection, $sql);
                mysqli_stmt_bind_param($stmt, "ssssss", $db_id, $time, $date, $time_suffix, $purpose_id, $schoolyear);
