...
<?php
    ...
    if(isset($_POST['studentID'])) {
        ...
        $query_student = "SELECT * FROM students WHERE student_id = ?";
        $stmt_student = mysqli_prepare($connection, $query_student);
        mysqli_stmt_bind_param($stmt_student, "s", $id);
        mysqli_stmt_execute($stmt_student);
        $result_student = mysqli_stmt_get_result($stmt_student);

        // Check if there's any record associated with the provided ID number in the faculty table
        $query_faculty = "SELECT * FROM faculty WHERE faculty_id = ?";
        $stmt_faculty = mysqli_prepare($connection, $query_faculty);
        mysqli_stmt_bind_param($stmt_faculty, "s", $id);
        ...
        if(mysqli_num_rows($result_student) > 0) {
            ...
            if(mysqli_num_rows($attendance_result) > 0){
                ...
                $_SESSION['matched_student_id'] = $id;
                redirect('home');
            } 
